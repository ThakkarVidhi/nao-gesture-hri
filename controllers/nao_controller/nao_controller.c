...
static WbMotionRef hand_wave, forwards, backwards, side_step_left, side_step_right, turn_left_60, turn_right_60, tai_chi,
  wipe_forehead, stand, neutral_stance, hand_on_chest, open_hand, pointing, count, thumbs_up, bow, good_bye;
...
static void load_motion_files() {
  ...
  stand = wbu_motion_new("../../motions/Stand.motion");
  neutral_stance = wbu_motion_new("../../motions/NeutralStance.motion");
  hand_on_chest = wbu_motion_new("../../motions/HandOnChest.motion");
  open_hand = wbu_motion_new("../../motions/OpenHand.motion");
  pointing = wbu_motion_new("../../motions/Pointing.motion");
  count = wbu_motion_new("../../motions/Count.motion");
  thumbs_up = wbu_motion_new("../../motions/ThumbsUp.motion");
  bow = wbu_motion_new("../../motions/Bow.motion");
  good_bye = wbu_motion_new("../../motions/GoodBye.motion");
}
...
// Idle function to wait before the next motion
static void idle(int seconds, int milliseconds) {
  double total_time = (seconds * 1000) + milliseconds;
  double start_time = wb_robot_get_time();
  printf("here");
  
  while ((wb_robot_get_time() - start_time) * 1000 < total_time) {
    printf("inside while");
    simulation_step();
  }
}

// Function to loop a motion for a specified duration
static void loop_motion(WbMotionRef motion, int seconds, int milliseconds) {
  // interrupt current motion
  if (currently_playing)
    wbu_motion_stop(currently_playing);

  // Enable looping for the specified motion
  ...
  // Wait for the specified duration while keeping the simulation running
  while ((wb_robot_get_time() - start_time) * 1000 < total_time) {
    simulation_step();
  }
  
  // Disable looping and stop the motion
  wbu_motion_set_loop(motion, false);

}

static void playSoundFile(const char *audioWav) {
  // Get the built-in NAO speaker
  WbDeviceTag speaker = wb_robot_get_device("speaker");
  if (speaker == 0) {
    printf("Error: Speaker device not found!\n");
    return;
  }
  ...
  // Play the sound
  wb_speaker_play_sound(speaker, speaker, audioWav, volume, pitch, balance, loop);
}
...
int main() {
  ...
  // play sound file
  playSoundFile("../../sounds/nao.wav");
  
  loop_motion(neutral_stance, 4, 0);
  idle(1, 500);
  
  loop_motion(hand_on_chest, 6, 0);
  
  loop_motion(neutral_stance, 4, 0);
  
  loop_motion(open_hand, 4, 0);
  
  loop_motion(stand, 1, 500);
  
  loop_motion(pointing, 6, 0);
  idle(1, 500);
  ...
  loop_motion(neutral_stance, 2, 0);
  idle(0, 500);
  
  loop_motion(count, 17, 0);
